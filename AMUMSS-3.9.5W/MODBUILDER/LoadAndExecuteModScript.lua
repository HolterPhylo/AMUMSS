...
function ExchangePropertyValue(item,file,FileTable,value_change_table,special_key_words,preceding_key_words
  ...
  local IsReplaceREPLACEATLINE = (add_option == "REPLACEATLINE") and IsTextToAdd
  
  local IsAddOption = (add_option ~= "")
  if IsAddOption then
    if IsTextToAdd and not (IsReplaceADDAFTERSECTION or IsReplaceADDAFTERLINE or IsReplaceREPLACEATLINE) then
  ...
  --this effectively deactivates SECTION_UP_PRECEDING
  ...
  -- *****************   ISxxx section   ********************
  local IsReplaceAllInGroup = IsReplace and ((IsPrecedingKeyWords and (not IsOneWordOnly)) or IsSpecialKeyWords)
  IsWholeFileSearch = (not IsPrecedingKeyWords and not IsSpecialKeyWords) or IsReplace and (not IsReplaceAllInGroup)
  ...
  if _mISxxx ~= nil then
    ...
    print(" +              IsTextToAdd: ["..tostring(IsTextToAdd)
              ...
              .."], IsReplaceREPLACEATLINE: ["..tostring(IsReplaceREPLACEATLINE).."]")
  ...
  if #GroupStartLine > 1 and not IsReplace then
    Report("","KEY_WORDS below located more than one section!","NOTICE")
    print(_zRED.."\n>>> [NOTICE] KEY_WORDS located more than one section!".._zDEFAULT)
  end
...
--END: the next 4 handlers sequence should be programmable !!!
  ...
  --recreate Group List and remove duplicates
    pv("ENDING FINGROUP SECTION:   Found "..#SectionActive.." SECTION_ACTIVE section(s)")
  GroupStartLine,GroupEndLine,SpecialKeyWordLine = RemoveDuplicateGroups(GroupStartLine,GroupEndLine,SpecialKeyWordLine)
  RecordSections(GroupStartLine,GroupEndLine,SpecialKeyWordLine,"Using ",SectionsTable)
    pv(" AFTER REMOVE DUPLICATE:   Found "..#SectionActive.." SECTION_ACTIVE section(s)")
...
--#########################################
      ...
      do --prepare info to inform user
        ...
        if IsTextToAdd then
          ...
          elseif IsReplaceADDAFTERSECTION then
            ...
            Report("","    Looking to >>> replace some text <<< at LINE with Property name ["..property.."] and value ["..value.."]"..msg3..msg4)
            print("\n    Looking to >>> replace some text <<< at LINE with Property name ["..property.."] and value ["..value.."]"..msg3..msg4)
            spacer = 11
          end
        ...
        elseif IsToRemove then
          ...
          elseif IsToRemoveSECTION then
            ...
            Report("","    Looking to >>> remove some text <<< at LINE with Property name ["..property.."] and value ["..value.."]"..msg3..msg4)
            print("\n    Looking to >>> remove some text <<< at LINE with Property name ["..property.."] and value ["..value.."]"..msg3..msg4)
            spacer = 11
          end
      ...
      if FoundNum > 0 then
        if FoundNum > 1 then
          Report("","    --                    >>>>> Found "..FoundNum.." valid candidate sections.")
          print("        >>>>> Found "..FoundNum.." valid candidate sections.")
          if IsReplaceALL then
            Report("","    --                    >>>>> ALL valid sections where requested to be processed <<<<<")
            print("\n    --                    >>>>> ALL valid sections where requested to be processed <<<<<")
          else
            Report("",[[    --     >>>>> REPLACE_TYPE is missing.  Only FIRST section will be used <<<<<]],"NOTICE")
            print("\n".._zRED..[[>>> [NOTICE] REPLACE_TYPE is missing.  Only FIRST section will be used]].._zDEFAULT)
      ...
      while k <= #GroupStartLine - 1 do
        ...
        while i <= (GroupEndLine[k] - 1) do
          if not InWhile then
            pv("   >>> Entering inner while at "..i.."...")
            --pv("This line: ["..FileTable[i].."]")
            InWhile = true
          end
          
          local repl_done = false
          if not IsReplaceREPLACEATLINE then
            i = i + 1 --next line
          end
          local CurrentLine = i --used with text_to_add and to_remove
          pv(">>> CurrentLine: "..i.."...")
          ...
          if IsReplaceRAW then
            ...
            -- or (property == "IGNORE" and (IsTextToAdd or IsToRemove)) --was this
            if       (i == 2) 
                  or StripInfo(line,[[<Property name="]],[["]]) == property 
                  or StripInfo(line,[[<Property value="]],[["]]) == property 
                  or (IsTextToAdd or IsToRemove)
                ...
                if not newIsValueMatchType 
                  ...
                  if not IsTextToAdd and not IsToRemove then
                    ...
                    if IsTextToAdd then
                      ...
                      if IsReplaceREPLACEATLINE then
                        pv("    -- Replacing/Adding text at line: "..i)
                    ...
                    if IsToRemove then
                      ...
                      if IsToRemoveSECTION then
                        ...
                        print(_zGREEN.."    -- Lines "..top.." - "..bottom.._zDEFAULT.." REMOVED")
          ...
          if repl_done then
            ...
            if not (IsTextToAdd or IsToRemove) then
              if value == "IGNORE" then
                ...
                local part2 = _zGREEN.."-- On line "..i.._zDEFAULT..", exchanged:" .. spacer1 .. "[" .. trim(line) .. "]"
                ...
                print(spacer..part2 .. spacer1 .. "with: " .. spacer2 .. "[" .. trim(FileTable[i]) .. "]")
  ...
  if success then --use this if using pcall above
    ...
    local function SerializeLoadedScript(TableName,thisTable,indentLevel,outTable)
      ...
      if tonumber(TableName) == nil then
        ...
        if TableName == "VALUE_CHANGE_TABLE" or TableName == "EXML_CHANGE_TABLE" or TableName == "MBIN_CHANGE_TABLE" or TableName == "MODIFICATIONS" then
          --for TABLE OF TABLES
          indentLevel = indentLevel + 1 --added
        end
      ...
      for k,v in pairs(thisTable) do
        ...
        if type(v) == "table" then
          ...
          SerializeLoadedScript(k,v,indentLevel,outTable) --recursive
          indentLevel = indentLevel - 1
        else
          if type(v) == "nil" then
            value = "nil,"
...
--***************************************************************************************************  
function ProcessScript(NMS_MOD_DEFINITION_CONTAINER,Multi_pak,_bScriptName)
  --Wbertro
  ...
  if not abortProcessing then
    ...
    local MOD_FILENAME = NMS_MOD_DEFINITION_CONTAINER["MOD_FILENAME"]
    if MOD_FILENAME == nil then
      MOD_FILENAME = "Unknown"
    end
    WriteToFileAppend("   MOD FILENAME: "..MOD_FILENAME.."\n",[[COMBINED_CONTENT_LIST.txt]])
    
    local MOD_AUTHOR = NMS_MOD_DEFINITION_CONTAINER["MOD_AUTHOR"]
    if MOD_AUTHOR == nil then
      MOD_AUTHOR = "Unknown"
    end
    WriteToFileAppend("     MOD AUTHOR: "..MOD_AUTHOR.."\n",[[COMBINED_CONTENT_LIST.txt]])
    
    local LUA_AUTHOR = NMS_MOD_DEFINITION_CONTAINER["LUA_AUTHOR"]
    if LUA_AUTHOR == nil then
      LUA_AUTHOR = "Unknown"
    end
    WriteToFileAppend("     LUA AUTHOR: "..LUA_AUTHOR.."\n",[[COMBINED_CONTENT_LIST.txt]])
    
    local MOD_DESCRIPTION = NMS_MOD_DEFINITION_CONTAINER["MOD_DESCRIPTION"]
    if MOD_DESCRIPTION == nil then
      MOD_DESCRIPTION = "Unknown"
    end   
    WriteToFileAppend("MOD DESCRIPTION: "..MOD_DESCRIPTION.."\n",[[COMBINED_CONTENT_LIST.txt]])
    
    local NMS_VERSION = NMS_MOD_DEFINITION_CONTAINER["NMS_VERSION"]
    if NMS_VERSION == nil then
      NMS_VERSION = "Unknown"
    end   
    WriteToFileAppend("    NMS VERSION: "..NMS_VERSION.."\n",[[COMBINED_CONTENT_LIST.txt]])
    ...
    if _bCOMBINE_MODS == "0" then
      ...
      if IsFileExist("MBINCompiler_error.txt") then
        local MBINCompilerFailedMsg = ParseTextFileIntoTable("MBINCompiler_error.txt")
    ...
    elseif _bNumberScripts == _bScriptCounter then
      ...
      print(_zRED..">>> [INFO] Combined Mod ACTIVE: Delaying Building MOD...".._zDEFAULT)
      Report("","Combined Mod ACTIVE: Delaying Building MOD...")
    end
...
--Only files in ModScript (no sub-directory)
...
for i=1,_bNumberScripts do
  ...
  print()
  print(_zBLACKonYELLOW.." >>> Starting to process script #".._bScriptCounter.." of ".._bNumberScripts
  ...
  if type(gNMS_MOD_DEFINITION_CONTAINER) == "table" then    
    ...
    if type(gNMS_MOD_DEFINITION_CONTAINER[1]) == "table" then
      ...
      for i=1,#Container do
        if i > 1 then
          print()
          print(_zRED..">>> Still processing script #".._bScriptCounter.." of ".._bNumberScripts.." [".._bScriptName.."]".._zDEFAULT)
          print()

          Report("")
          Report("","========================================================================================")
          Report("","Still processing script #".._bScriptCounter.." of ".._bNumberScripts.." [".._bScriptName.."]")
        else
          Report("")
          Report("","========================================================================================")
          Report("","Processing script #".._bScriptCounter.." of ".._bNumberScripts.." [".._bScriptName.."]")
        end
        ...
        ProcessScript(Container[i],True,_bScriptName)
      ...
      Report("")
      Report("","========================================================================================")
      Report("","Starting to process script #".._bScriptCounter.." of ".._bNumberScripts.." [".._bScriptName.."] {")
      ...
      ProcessScript(gNMS_MOD_DEFINITION_CONTAINER,false,_bScriptName)
    ...
    Report("","Check the 'Analysis section' of the cmd window or the log.txt file for hints","  >>>")
  end
  ...
  if _bCOMBINE_MODS ~= "0" then
    ...
    if _bNumberScripts == _bScriptCounter then
      ...
      if string.sub(_bCOPYtoNMS,1,1) ~= "N" then
