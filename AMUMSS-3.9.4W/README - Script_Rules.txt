...
GLOBAL_INTEGER_TO_FLOAT = {
	...
	- WARNING: even if you use this option to silence the AMUMSS WARNING's, MBINCompiler may still refuse
		...
		USE 'GLOBAL_INTEGER_TO_FLOAT' if you know that what you are doing is right
...
WHERE_IN_SECTION = {
	...
	- 2 Cases:	
		...
		A) IF no "Property name/value" with that "WhereValue" is found in the section 
			- then that SECTION is skipped/ignored
		B) IF a "Property name/value" has that "WhereValue" in the section 
			- then that SECTION will be used by the script to update the EXML

	- WHERE_IN_SUBSECTION is processed after WHERE_IN_SECTION if also present
...
WHERE_IN_SUBSECTION = {
	...
	- 3 Cases:	
		For each section specified by SPECIAL_KEY_WORDS and/or PRECEDING_KEY_WORDS:
		...
		C) IF a "Property name/value" has that "WhereValue" in a SUB-SECTION of the section 
			- then that SUB-SECTION will be used by the script to update the EXML 
...
REPLACE_TYPE = {
	...
	- OBSOLETE: Using ["REPLACE_TYPE"] = "ADDafterSECTION", is now 'OBSOLETE' (but kept for backward compatibility)
...
NEW	- can have one of these values: {nil, "", "ADDatLINE", "ADDafterLINE", "ADDafterSECTION"}
		ex.: 	["ADD_OPTION"] 	= "ADDatLINE",
			or	["ADD_OPTION"] 	= "ADDafterLINE", -- default
			or	["ADD_OPTION"] 	= "ADDafterSECTION",
				
	- == if missing or nil or "": "ADDafterLINE" is the default behavior

NEW	- == "ADDatLINE" : used with ADD, specifies to add (meaning replacing that line) TEXT_TO_ADD 'at' the LINE
