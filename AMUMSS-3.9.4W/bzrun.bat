...
if %_bOS_bitness%==64 (
	echo|set /p="[INFO] on %_bWinVer% 64bit, Build: %_bCurrentBuildDec%.%_bUBRDEC% with %NUMBER_OF_PROCESSORS% logical CPUs (cp%_CodePage%)">>"REPORT.txt" & echo.>>"REPORT.txt"
) else (
	echo|set /p="[INFO] on %_bWinVer% 32bit, Build: %_bCurrentBuildDec%.%_bUBRDEC% with %NUMBER_OF_PROCESSORS% logical CPUs (cp%_CodePage%)">>"REPORT.txt" & echo.>>"REPORT.txt"
)
...
if DEFINED _bPAK_MBIN (
	...
	if %_bNumberMBINs% GTR 1 (
		...
		echo|set /p="[INFO] Detected %_bNumberPAKs% user MBINs in ModScript...">>"REPORT.txt" & echo.>>"REPORT.txt"
	) else (
		if %_bNumberMBINs% GTR 0 (
			echo. %_zBLACKonYELLOW% ^>^>^> Detected 1 user MBIN in ModScript... %_zDEFAULT%
			echo.

			echo|set /p="[INFO] Detected 1 user MBIN in ModScript...">>"REPORT.txt" & echo.>>"REPORT.txt"
		)
	)
	...
	if %_bNumberScripts% EQU 0 (
		...
		echo|set /p="[NOTE] Placing one or more paks/mbins in Modscript, without a .lua script will unpack and decompile them">>"REPORT.txt" & echo.>>"REPORT.txt"
		...
		echo.>>"REPORT.txt"
		echo|set /p="[NOTE] FOR THIS TO WORK, the pak(s) MUST be fully updated to the current NMS files">>"REPORT.txt" & echo.>>"REPORT.txt"
		echo|set /p="[NOTE]   since the MBIN files in the pak are used to create the patch">>"REPORT.txt" & echo.>>"REPORT.txt"
		echo.>>"REPORT.txt"
...
if %_uOldMBINCompilerFlag%==Y (
	...
	echo.>>"REPORT.txt"
	echo|set /p=".   [NOTICE] Older MBINCompiler used">>"REPORT.txt" & echo.>>"REPORT.txt"
...
if %_bNumberScripts% GTR 0 (
	...
	echo.>>"REPORT.txt"
	echo|set /p="[INFO] Updated EXML_Helper\MODDED">>"REPORT.txt" & echo.>>"REPORT.txt"
	...
	if %_bNumberScripts% GTR 1 (
		...
		echo|set /p="[INFO]     Note that the MODDED files ARE based on the last processed script if individual mods were created">>"REPORT.txt" & echo.>>"REPORT.txt"
	)
...
echo.   %_zBLACKonYELLOW% ^>^>^>        AMUMSS v%_mMasterVersion% finished        ^<^<^< %_zDEFAULT%
echo.%_zDARKGRAY%-----------------------------------------------------%_zDEFAULT%
echo.
...
if defined _bNoScript (
	if %_bNumberPAKs% EQU 0 (
		...
		echo|set /p=".   [NOTICE] No user .lua Mod Script found in ModScript...">>"REPORT.txt" & echo.>>"REPORT.txt"
	...
	if not defined _bErrorLoadingScript (
		...
		echo|set /p="[INFO] Total scripts processed: %_bScriptCounter%">>"REPORT.txt" & echo.>>"REPORT.txt"
		echo.>>"REPORT.txt"
		echo.>>"REPORT.txt"
	)
)
...
if !_bCheckMODSconflicts! NEQ 2 (
	...
	if !_bGConflictLines! GTR 0 (
		...
		if !_bCheckMODSconflicts! EQU 3 (
			echo|set /p="[INFO] Only checking conflicts in MODS, at user request (no script processed)">>"REPORT.txt" & echo.>>"REPORT.txt"
		)
		
		if !_bCheckMODSconflicts! EQU 4 (
			echo|set /p="[INFO] Only checking conflicts in ModScript, at user request">>"REPORT.txt" & echo.>>"REPORT.txt"
		)
	...
	echo.
	echo|set /p="[INFO] Skipped Conflict Detection at user request">>"REPORT.txt" & echo.>>"REPORT.txt"
)

echo.              %_zBLACKonYELLOW% ^>^>^> FINAL REPORT ^<^<^< %_zDEFAULT%
echo.            %_zBLACKonYELLOW% ^>^>^> See "REPORT.txt" ^<^<^< %_zDEFAULT%
...
if defined _bUNPACKED_DECOMPILED (
	...
	echo.>>"REPORT.txt"
	echo|set /p="[INFO] %_bNumberMBINs% MBIN(s) processed">>"REPORT.txt" & echo.>>"REPORT.txt"
	echo|set /p="[INFO] %_bGNumScriptsInPak% Script(s) found in PAK(s)">>"REPORT.txt" & echo.>>"REPORT.txt"
	echo|set /p="[INFO] %_bNumberPAKs% PAK(s)/MBIN(s) processed">>"REPORT.txt" & echo.>>"REPORT.txt"
	echo|set /p="[INFO]   %_bGNumberFiles% file(s) found in PAK(s)/MBIN(s)">>"REPORT.txt" & echo.>>"REPORT.txt"
...
:LuaEndedOk
	if not EXIST  "%_bMASTER_FOLDER_PATH%MODBUILDER\LuaEndedOK.txt" (
		...
		echo.    [BUG] lua.exe generated an [ERROR]... Please report ALL scripts AND this file to NMS Discord: "No Man's Sky Modding" channel, "mod-amumss-lua" room:>>"%_bMASTER_FOLDER_PATH%REPORT.txt"
...
:MBINCompilerUPDATE
	...
	if [%_bMBINCompilerVersion%]==[] (
		...
		echo|set /p="[ERROR] MBINCompiler.exe cannot execute.  Most probable cause: anti-virus, make exception">>"..\REPORT.txt" & echo.>>"..\REPORT.txt"
		echo.>>"..\REPORT.txt"
		pause
		exit
	)
...
:UNPACKEDtoEXML
	...
	FOR /r ".\EXTRACTED_PAK\" %%G in (*.mbin.*) do (
		...
		if "!_gMBINVersion!"=="0.0.0.0" (
			...
			if "!_gMBINVersion!"=="" (
				...
				echo|set /p=".   [NO VERSION INFO]    Could not check version: !_bNMSname!">>"..\REPORT.txt" & echo.>>"..\REPORT.txt"
				...
				if DEFINED _bDoingPAk echo|set /p="[INFO]     Compiled with version !_gMBINVersion!: !_bNMSname!">>"REPORT_!_bPAKname!.txt" & echo.>>"REPORT_!_bPAKname!.txt"
			)
		)
	)
	echo.
	...
	if DEFINED _bDoingPAk echo|set /p="[INFO]   Trying to decompile .mbin...">>"REPORT_!_bPAKname!.txt" & echo.>>"REPORT_!_bPAKname!.txt"
	...
	FOR /r ".\EXTRACTED_PAK\" %%G in (*.mbin.*) do (
		...
		if exist !_gEXML_FILE! (
			...
			if DEFINED _bDoingPAk echo|set /p="[INFO]     SUCCESS: Decompiled with current MBINCompiler.%_bMBINCompilerVersion%: !_bNMSname!">>"REPORT_!_bPAKname!.txt" & echo.>>"REPORT_!_bPAKname!.txt"
			...
			if !_uFound!==Y (
				...
				if DEFINED _bDoingPAk echo|set /p="[INFO]     SUCCESS: Decompiled with ------> !_bCurrentCompiler!: !_bNMSname!">>"REPORT_!_bPAKname!.txt" & echo.>>"REPORT_!_bPAKname!.txt"
				SET /A _bNumberFilesDecompiled=_bNumberFilesDecompiled+1
			) else (
				...
				echo|set /p="[INFO]       SORRY:                 No MBINCompiler could decompile this file: !_bNMSname!">>"..\REPORT.txt" & echo.>>"..\REPORT.txt"
				if DEFINED _bDoingPAk echo|set /p="[INFO]       SORRY:                 No MBINCompiler could decompile this file: !_bNMSname!">>"REPORT_!_bPAKname!.txt" & echo.>>"REPORT_!_bPAKname!.txt"
			)
			set _zUOLE=
	...
	if DEFINED _bDoingPAk echo|set /p="[INFO]   Decompiled %_bNumberFilesDecompiled% / %_bNumberFiles% files">>"REPORT_!_bPAKname!.txt" & echo.>>"REPORT_!_bPAKname!.txt"
	...
	if %_bNumScriptsInPak% GTR 0 (
		if %_bNumScriptsInPak% EQU 1 (
			...
			echo|set /p="[INFO]   Copied one script file">>"..\REPORT.txt" & echo.>>"..\REPORT.txt"
			if DEFINED _bDoingPAk echo|set /p="[INFO]   Copied one script file">>"REPORT_!_bPAKname!.txt" & echo.>>"REPORT_!_bPAKname!.txt"
		) else (
			echo.%_zGREEN%^>^>^> Copied %_bNumScriptsInPak% script files %_zDEFAULT%
			echo|set /p="[INFO]   Copied %_bNumScriptsInPak% script files">>"..\REPORT.txt" & echo.>>"..\REPORT.txt"
			if DEFINED _bDoingPAk echo|set /p="[INFO]   Copied %_bNumScriptsInPak% script files">>"REPORT_!_bPAKname!.txt" & echo.>>"REPORT_!_bPAKname!.txt"
		)
	)
