...
function GetFileSize(filename)
  local filehandle = assert(io.open(filename,"r"),"io.open: Cannot open file to get file size: "..filename)
...
function WriteToFile(output,filename,binary)
  ...
  if binary == "b" then
    ...
    filehandle = assert(io.open(filename,"wb"),"io.open: Cannot open binary file to write: "..filename)
  else
...
--***************************************************************************************************
function WriteToFileAppendEXT(filename,...)
...
function LoadFileData(filename,binary)
  ...
  if IsFileExist(filename) then
    if binary == "b" then
      filehandle = assert(io.open(filename,"rb"),"io.open: Cannot open binary file to load: "..filename)
    else
...
function Report(Info,msg,msgType)
  ...
  -- if gfilePATH == "..\\" and FileName == "" then
    -- -- this is the standard Report.txt file
