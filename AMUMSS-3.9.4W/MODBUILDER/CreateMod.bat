...
if [] == [!_cMOD_FILENAME!] (
	...
	echo|set /p=".   [WARNING]     No MOD filename. Not creating pak file for this script">>"..\REPORT.txt" & echo.>>"..\REPORT.txt"
	goto :ENDIT
) else (
...
rem check if we have really something to pak beside the script
SET _cNumFiles=0
...
if %_cNumFiles% EQU 0 (
	...
	echo|set /p=".   [NOTICE] Nothing to pak. Not creating pak file for this script">>"..\..\REPORT.txt" & echo.>>"..\..\REPORT.txt"
	goto :ENDIT
)
...
echo.
REM echo|..\psarc.exe create --overwrite --skip-missing-files --inputfile=..\input.txt --output="%_cDestination%\%_cFilename%(%_ca%).pak%_cMOD_AUTHOR%"
REM rem Contributed by mso53 on Discord
...
xcopy /s /y /h /v /j "%_cDestination%\%_cFilename%(%_ca%).pak%_cMOD_AUTHOR%" "..\..\Builds\%_cMOD_FILENAME%%_cMOD_AUTHOR%*" 1>NUL 2>NUL
...
xcopy /s /y /h /v /j "..\COMBINED_CONTENT_LIST.txt" "..\..\Builds\%_cMOD_FILENAME%%_cMOD_AUTHOR%_content.txt*" 1>NUL 2>NUL
...
xcopy /s /y /h /v /j "%_cDestination%\%_cFilename%(%_ca%).pak%_cMOD_AUTHOR%" "..\..\Builds\%_cFilename%(%_ca%).pak%_cMOD_AUTHOR%*" 1>NUL 2>NUL
...
xcopy /s /y /h /v /j "..\COMBINED_CONTENT_LIST.txt" "..\..\Builds\%_cFilename%(%_ca%).pak%_cMOD_AUTHOR%_content.txt*" 1>NUL 2>NUL
